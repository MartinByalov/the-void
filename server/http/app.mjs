...
import {getTrades,publishTrade,proposeTrade,confirmTrade,cancelTrade} from "../trades/service.mjs";
import {getChat,postChat} from "../chat/service.mjs";
const web=path.resolve("web/public"),port=Number(process.env.PORT||3000),host=process.env.HOST||"0.0.0.0",production=process.env.NODE_ENV==="production";
...
const secure=res=>{res.setHeader("X-Content-Type-Options","nosniff");res.setHeader("Referrer-Policy","strict-origin-when-cross-origin");res.setHeader("Permissions-Policy","camera=(), microphone=(), geolocation=()")};
 ...
 try{
  ...
  if(u.pathname==="/api/lot"&&req.method==="GET")return json(res,200,getLot(sid));
   ...
   if(u.pathname==="/api/chat"&&req.method==="GET")return json(res,200,getChat(sid));
    if(u.pathname==="/api/chat"&&req.method==="POST"){let b=await body(req);return safe(()=>json(res,200,postChat(sid,b.text,b.mode)),res)}
  ...
  if(u.pathname==="/api/heartbeat"&&req.method==="POST")return json(res,200,heartbeat(sid));
   if(u.pathname==="/api/archive"&&req.method==="GET")return json(res,200,archive().map(x=>({...x.record,claimable:x.winner_session===sid})));
   let ar=u.pathname.match(/^\/api\/archive\/([^/]+)$/);if(ar&&req.method==="GET"){let x=findArtifact(decodeURIComponent(ar[1]));return x?json(res,200,{...x.record,claimable:x.winner_session===sid}):json(res,404,{error:"NOT_FOUND"})}
  if(u.pathname==="/api/trades"&&req.method==="GET")return json(res,200,getTrades(sid));
  if(u.pathname==="/api/trades/publish"&&req.method==="POST"){let b=await body(req);return json(res,200,publishTrade(sid,b.artifact,b.signature))}
  let trProp=u.pathname.match(/^\/api\/trades\/([^/]+)\/propose$/);if(trProp&&req.method==="POST"){let b=await body(req);return json(res,200,proposeTrade(trProp[1],sid,b.offered_artifact))}
  let trConf=u.pathname.match(/^\/api\/trades\/([^/]+)\/confirm$/);if(trConf&&req.method==="POST"){let b=await body(req);return json(res,200,confirmTrade(trConf[1],sid,b.role))}
  let trCanc=u.pathname.match(/^\/api\/trades\/([^/]+)\/cancel$/);if(trCanc&&req.method==="POST")return json(res,200,cancelTrade(trCanc[1],sid));
  ...
  let ext=path.extname(fp),ct={".html":"text/html; charset=utf-8",".js":"text/javascript; charset=utf-8",".css":"text/css; charset=utf-8",".svg":"image/svg+xml"}[ext]||"application/octet-stream";res.writeHead(200,{"content-type":ct});fs.createReadStream(fp).pipe(res)
