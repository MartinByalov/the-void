...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Destructive admin maintenance: clear canonical Registry and reset serial to VA-000001.
      if (url.pathname === "/api/vault/registry/clear" && request.method === "POST") {
        ...
        if (warp.state !== "IDLE") {
          ...
          warp = {
            ...createEmptyWarpState(),
            warpId: Number(warp.warpId || 0),
            lastOutcome: warp.lastOutcome || null,
          };
          await saveWarpState(env, warp);
          const hub = getWarpHub(env);
          try { await hub.fetch(new Request("https://warp.internal/reset", { method: "POST" })); } catch {}
          await armAndBroadcast(env, warp);
        }

        let driveResult = null;
        try {
          driveResult = await drivePost(env, { action: "clearRegistry" });
        } catch (err) {
          console.warn("drivePost clearRegistry warning:", err);
        }

        // Invalidate all potential image caches in KV (from VA-000001 to VA-000500)
        try {
          const list = await env.VOID_KV.list({ prefix: "IMG_CACHE_" });
          if (list?.keys?.length) {
            await Promise.all(list.keys.map(k => env.VOID_KV.delete(k.name)));
          }
        } catch (err) {
          console.warn("KV image cache clear warning:", err);
        }
        ...
        return json({ success: true, count: 0, nextSerial: 1, deletedImages: Number(driveResult?.deletedImages || 0) });
      }
      ...
      if (url.pathname === "/api/vault/registry/remove-last" && request.method === "POST") {
        ...
        // Invalidate image cache in KV for the removed artifact
        try {
          await Promise.all([
            env.VOID_KV.delete(`IMG_CACHE_${id}`),
            env.VOID_KV.delete(`IMG_CACHE_${id.replace(/-/g, "_")}`),
            env.VOID_KV.delete(`IMG_CACHE_${id.replace(/_/g, "-")}`)
          ]);
        } catch {}
      ...
      if (url.pathname === "/api/archive/clear" && request.method === "POST") {
        ...
        try {
          await drivePost(env, { action: "writeArchive", entries: [] });
        } catch (e) {
          console.warn("drivePost writeArchive clear warning:", e);
        }
        return json({ success: true, count: 0, updatedAt: Date.now() });
      }
      ...
      // Manual STOP control: transitions WAITING or ACTIVE -> IDLE and resets alarms.
      if (url.pathname === "/api/warp/stop" && request.method === "POST") {
        let warp = await getWarpState(env);
        if (warp.nextArtifact) {
          const registry = await getRegistry(env);
          const nextId = getArtifactId(warp.nextArtifact);
          if (nextId && !registry.queue.some(x => getArtifactId(x) === nextId)) {
            registry.queue.unshift(warp.nextArtifact);
            await saveRegistry(env, registry);
          }
        }
        await env.VOID_KV.delete(ACTIVE_SPAWN_KEY);
        ...
        await hub.fetch(new Request("https://warp.internal/reset", { method: "POST" }));
        await pushWarpFeed(env, {
          id: `warp-${warp.warpId}-stop-${Date.now()}`,
          type: "WARP_STOPPED",
          at: Date.now(),
          title: "THE VOID HAS SETTLED.",
          text: "WARP cycle stopped by administrator.",
        });
        await armAndBroadcast(env, warp);
        return json({ success: true, stopped: true, serverNow: Date.now(), warp: adminWarp(warp) });
      }

      // Test-only or manual forced drop. Surfaces either selected or next sequential Artifact immediately.
      if (url.pathname === "/api/warp/force-drop" && request.method === "POST") {
        ...
        if (body.artifactId) {
          const targetId = String(body.artifactId).trim();
          if (warp.nextArtifact && getArtifactId(warp.nextArtifact) === targetId) {
            targetArtifact = warp.nextArtifact;
            warp.nextArtifact = null;
          } else {
            const idx = registry.queue.findIndex(x => getArtifactId(x) === targetId);
            if (idx >= 0) {
              targetArtifact = registry.queue.splice(idx, 1)[0];
              await saveRegistry(env, registry);
            } else {
              const driveReg = await driveGet(env, "registry").catch(() => null);
              const driveArts = Array.isArray(driveReg?.registry?.artifacts) ? driveReg.registry.artifacts : [];
              const match = driveArts.find(x => getArtifactId(x) === targetId);
              if (match) targetArtifact = match;
            }
          }
          if (!targetArtifact) {
            return json({ success: false, error: "ARTIFACT_NOT_FOUND", message: `Artifact ${targetId} is not in queue.` }, 404);
          }
        } else {
          if (warp.state === "WAITING" && warp.nextArtifact) {
            targetArtifact = warp.nextArtifact;
            warp.nextArtifact = null;
          } else if (registry.queue.length > 0) {
            targetArtifact = registry.queue.shift();
            await saveRegistry(env, registry);
          } else {
            const driveReg = await driveGet(env, "registry").catch(() => null);
            const driveArts = Array.isArray(driveReg?.registry?.artifacts) ? driveReg.registry.artifacts : [];
            const unspawned = driveArts.filter(x => !registry.spawnedHistory.includes(getArtifactId(x)));
            if (unspawned.length > 0) {
              targetArtifact = unspawned[0];
            } else if (driveArts.length > 0) {
              targetArtifact = driveArts[0];
            } else {
              return json({ success: false, error: "VAULT_EMPTY", message: "No artifacts registered in Vault." }, 409);
            }
          }
        }

        warp = await activateArtifact(env, warp, targetArtifact, now);
        await armAndBroadcast(env, warp);
