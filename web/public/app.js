...
const sum = (n, v) => `<div><b>${v}</b><small>${n}</small></div>`;
 function drawArchive(a) { let g = $("#agrid"); g.innerHTML = ""; if (!a.length) { g.innerHTML = '<div class="empty archive-empty" aria-label="No artifacts"><span class="void-orb">◉</span></div>'; return } for (let x of a) { let c = document.createElement("article"); c.className = "artifactcard flip-card archive-artifact-card"; c.innerHTML = `<div class="card-face card-front"><div class=cardart></div><div class=cardbody><small class=card-class>${esc(x.classification)}</small><b class="card-name rarity-name ${esc(x.rarity)}">${esc(x.name)}</b><small class=card-weirdness>👁 Weirdness ${esc(x.weirdness)}</small></div></div><div class="card-face card-back"><span class="void-orb-mark">◉</span><small class="archive-back-id">#${esc(x.artifact_id)}</small></div>`; c.querySelector(".cardart").append(cv(x)); c.onclick = () => c.classList.toggle("is-flipped"); let nb = c.querySelector(".card-name"); if (nb) nb.onclick = e => { e.stopPropagation(); history.pushState({}, '', `/archive/${encodeURIComponent(x.artifact_id)}`); collectionDetail(x) }; g.append(c) } }
 ...
 const weirdnessRow = value => `<div class=recordrow><span>WEIRDNESS</span><span class="detail-weirdness ${weirdnessTier(value)}">${esc(value)}</span></div>`;
...
function drawTrades(trades) {
  ...
  for (let t of trades) {
    ...
    c.className = "artifactcard flip-card collection-card trade-card";
    let statusBadge = t.status === "PENDING"
      ? '<span class="trade-card-status status-pending">⚡ TRADE PENDING</span>'
      : t.status === "COMPLETED"
    ...
    c.innerHTML = `<div class="card-face card-front"><div class=cardart></div><div class=cardbody><small class=card-owner>${ownerLabel}</small><small class=card-class>${esc(t.artifact.classification)}</small><b class="card-name rarity-name ${esc(t.artifact.rarity)}">${esc(t.artifact.name)}</b><small class=card-weirdness>👁 Weirdness ${t.artifact.weirdness}</small><div class="trade-card-footer">${statusBadge}</div></div></div><div class="card-face card-back"><span class="void-orb-mark">◉</span><small class="trade-back-id">#${esc(t.artifact.artifact_id)}</small></div>`;
    c.querySelector(".cardart").append(cv(t.artifact));
    c.onclick = () => c.classList.toggle("is-flipped");
    c.querySelector(".card-name").onclick = e => { e.stopPropagation(); openTradeModal(t) };
    g.append(c);
  }
}

function renderSideCard(a, headerTitle, statusBadgeHtml) {
  return `<div class="trade-col-header"><small>${esc(headerTitle)}</small>${statusBadgeHtml}</div><div class="card-title-group" style="margin-bottom:6px"><h3 class="rarity-name ${esc(a.rarity)}" style="margin:2px 0 0;font-size:16px">${esc(a.name)}</h3></div><div class="card-art-container" style="margin-bottom:8px"><div class="detailart flip-card detail-flip-card" id="art-${esc(a.artifact_id)}" style="cursor:pointer"><span class="card-face card-front"></span><span class="card-face card-back"><span class="void-orb-mark">◉</span><small class="detail-back-id">#${esc(a.artifact_id)}</small></span></div></div><div class="collection-stats">${rr("CLASSIFICATION", a.classification)}${rr("ANOMALY", String(a.anomaly || "UNKNOWN").replaceAll("_", " "))}${weirdnessRow(a.weirdness)}</div>`;
}
...
function openTradeModal(trade) {
  ...
  const renderModalContent = (st, off) => {
    let leftStatusHtml = (st === "PENDING" && trade.creator_confirmed)
      ? '<span class="party-status ready">✓ PUBLISHER READY</span>'
      : (st === "PENDING" ? '<span class="party-status waiting">AWAITING PARTNER</span>' : '<span class="party-status ready">PUBLISHED</span>');
    let rightStatusHtml = off
      ? (trade.respondent_confirmed ? '<span class="party-status ready">✓ CONFIRMED</span>' : '<span class="party-status waiting">PENDING CONFIRMATION</span>')
      : '<span class="party-status waiting">NO OFFER YET</span>';
    ...
    if (trade.is_owner) {
      ...
      rightColContent = `<div class="trade-empty-picker"><h3>CHOOSE ARTIFACT TO OFFER</h3><p>Select an authentic artifact from your collection or drop a .png file to negotiate side-by-side.</p><div class="trade-offer-actions"><button class="join" id="btn-choose-offer-collection" type="button">COLLECTION</button><button class="understood-button" id="btn-upload-offer-png" type="button">DROP ARTIFACT</button></div></div>`;
    }
...
function openTradeCollectionPicker(excludedArtifactId, onSelect) {
  ...
  const draw = () => {
    ...
    grid.innerHTML = filtered.length ? filtered.map(x => `<button class="trade-collection-item" type="button" data-id="${esc(x.artifact_id)}"><b class="rarity-name ${esc(x.rarity)}">${esc(x.name)}</b><small>#${esc(x.artifact_id)} · ${esc(x.classification)} · 👁 ${esc(x.weirdness)}</small></button>`).join("") : '<div class="empty">NO MATCHING ARTIFACTS.</div>';
...
function drawCollection() { if (!$("#csum")) return; $("#csum").innerHTML = sum("ARTIFACTS", local.length) + sum("TOTAL WEIRDNESS", local.reduce((s, x) => s + x.weirdness, 0)) + sum("LEGENDARY+", local.filter(x => ["LEGENDARY", "MYTHIC", "DIVINE"].includes(x.rarity)).length) + sum("CLASSES", new Set(local.map(x => x.classification)).size); $("#sets").innerHTML = '<h3>SECRET SETS</h3>' + secretSets.map(set => `<button class="set set-filter ${set.complete(local) ? "unlocked" : ""} ${selectedSecretSet === set.name ? "active" : ""}" type="button" data-secret-set="${esc(set.name)}" aria-pressed="${selectedSecretSet === set.name}">${esc(set.name)}</button>`).join(""); document.querySelectorAll("[data-secret-set]").forEach(button => button.onclick = () => { let name = button.getAttribute("data-secret-set"); selectedSecretSet = selectedSecretSet === name ? "" : name; drawCollection() }); drawLocalCards() }
 function drawLocalCards() { let g = $("#cgrid"); if (!g) return; let q = $("#collection-q")?.value.toLowerCase() || "", order = $("#collection-sort")?.value || "asc", r = $("#collection-r")?.value || "", cl = $("#collection-c")?.value || "", weirdness = $("#collection-weirdness")?.value || "", secretSet = secretSets.find(set => set.name === selectedSecretSet), items = local.filter(x => { let [minimum, maximum] = weirdness ? weirdness.split("-").map(Number) : [0, 99]; return (!q || `${x.name} ${x.artifact_id}`.toLowerCase().includes(q)) && (!r || x.rarity === r) && (!cl || x.classification === cl) && (!weirdness || (x.weirdness >= minimum && x.weirdness <= maximum)) && (!secretSet || secretSet.matches(x)) }); items.sort((a, b) => order === "desc" ? b.name.localeCompare(a.name) : a.name.localeCompare(b.name)); g.innerHTML = ""; if (!items.length) { g.innerHTML = '<div class="empty collection-empty" aria-label="No artifacts"><span class="void-orb">◉</span></div>'; return } for (let x of items) { let c = document.createElement("article"); c.className = "artifactcard flip-card collection-card"; c.innerHTML = `<div class="card-face card-front"><div class=cardart></div><div class=cardbody><small class=card-class>${esc(x.classification)}</small><b class="card-name rarity-name ${esc(x.rarity)}">${esc(x.name)}</b><small class=card-weirdness>👁 Weirdness ${x.weirdness}</small></div></div><div class="card-face card-back"><span class="void-orb-mark">◉</span><small class="collection-back-id">#${esc(x.artifact_id)}</small></div>`; c.querySelector(".cardart").append(cv(x)); c.onclick = () => c.classList.toggle("is-flipped"); let nb = c.querySelector(".card-name"); if (nb) nb.onclick = e => { e.stopPropagation(); collectionDetail(x) }; g.append(c) } }
function collectionDetail(a) {
  ...
  app.insertAdjacentHTML("beforeend", `<div class=modal id=collection-modal><div class="collection-detail-modal"><div class="panel modalbox collection-detail vertical-card-modal archive-detail-card"><div class=card-header><button class="archive-close collection-close" id=collection-x type=button aria-label="Close collection artifact"><svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><line x1="2" y1="2" x2="12" y2="12"></line><line x1="12" y1="2" x2="2" y2="12"></line></svg></button><div class=card-title-group><h2 class="rarity-name ${esc(a.rarity)}">${esc(a.name)}</h2></div></div><div class=card-info><div class="card-art-container collection-detail-art"><div class="detailart flip-card detail-flip-card" id=collection-detail-art><span class="card-face card-front"></span><span class="card-face card-back"><span class="void-orb-mark">◉</span><small class="detail-back-id">#${esc(a.artifact_id)}</small></span></div></div><p class=lore>${esc(a.lore || "")}</p><div class=collection-stats>${rr("CLASSIFICATION", a.classification)}${rr("ANOMALY", String(a.anomaly || "UNKNOWN").replaceAll("_", " "))}${weirdnessRow(a.weirdness)}${(a.stats || []).map(s => rr(s.name, `${s.value}${s.unit || ""}`)).join("")}</div></div></div><div class=collection-detail-actions>${actions}</div></div></div>`);
