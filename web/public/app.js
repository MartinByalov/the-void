    ...
    import { deriveArtifactFunctions, getArtifactBySerial, generateBatchRegistry } from "./void-lexicon.js";
    ...
    let hasTriggeredLootExpiryCheck = false;
    ...
    async function voidHome() {
      ...
      hasTriggeredLootExpiryCheck = false;
      presenceTimer = setInterval(async () => {
        if (!current?.revealed) {
          if (warpStateClosed) {
            ...
            if (lootSecondsRemaining <= 0) {
              if (!hasTriggeredLootExpiryCheck && !warpCheckInProgress) {
                hasTriggeredLootExpiryCheck = true;
                await checkPublicWarpOnce();
              }
            } else {
              hasTriggeredLootExpiryCheck = false;
    ...
    // JOIN THE WARP elapsed-time counter.
    // It starts at 00:00 and counts upward locally from the moment the user clicks JOIN THE WARP.
    ...
    function getJoinedWarpElapsedSeconds() {
      ...
      return Math.max(0, Math.floor((Date.now() - warpJoinedAt) / 1000));
    }
    ...
    function applyPublicWarpState(state) {
      ...
      const prevActive = warpDropActive;
      const prevArtifactId = activeWarpArtifact?.id || activeWarpArtifact?.artifact_id || null;
      ...
      if (state.state === "ACTIVE" && state.artifact) {
        ...
        const currentArtifactId = activeWarpArtifact.id || activeWarpArtifact.artifact_id || null;
        const stateChanged = !prevActive || prevArtifactId !== currentArtifactId;
    ...
    function drawLotPart(l) {
      ...
      let currentLotKey = "";
      if (l.revealed) {
        currentLotKey = `revealed:${l.revealed.id || l.revealed.artifact_id}:${l.winner ? 1 : 0}`;
      } else if (l.found_owner) {
        currentLotKey = `owner:${l.surfaced_name || ""}:${l.next_warp?.id || ""}`;
      } else if (warpDropActive && activeWarpArtifact) {
        currentLotKey = `active_drop:${activeWarpArtifact.id || activeWarpArtifact.artifact_id}`;
      } else {
        currentLotKey = `idle:${l.id}:${warpStateClosed ? 'closed' : 'waiting'}:${joinedWarpGimmick ? 'joined' : 'unjoined'}`;
      }

      // If already rendered with this exact state/artifact key, only update dynamic texts/buttons to prevent image reloading flicker!
      if (st.dataset.stageKey === currentLotKey && d.dataset.detailsKey === currentLotKey) {
        if (warpDropActive) {
          const lootTimer = $("#loot-timer");
          if (lootTimer) lootTimer.textContent = `${lootSecondsRemaining}s`;
        } else if (warpStateClosed) {
          const spawnClock = $("#spawn-clock");
          if (spawnClock) spawnClock.textContent = "WARP CLOSED";
        } else if (joinedWarpGimmick) {
          const spawnClock = $("#spawn-clock");
          if (spawnClock) spawnClock.textContent = `◉ WARP ${formatWarpElapsed(getJoinedWarpElapsedSeconds())}`;
        }
        return;
      }

      st.dataset.stageKey = currentLotKey;
      d.dataset.detailsKey = currentLotKey;
      st.innerHTML = "";
      ...
      if (l.revealed) {
        ...
        const warpFunctions = warpDropActive ? deriveArtifactFunctions(activeWarpArtifact) : null;
        d.innerHTML = `<div class=lot-kicker>CURRENT WARP #${l.id}</div><p>${warpDropActive ? "✦ AN ARTIFACT HAS DROPPED! LOOT WINDOW ACTIVE (1 MINUTE)!" : (warpStateClosed ? "No artifacts active in The Void queue." : "Its true nature is hidden... for now.")}</p><div class="statbox current-lot-stats">${kv("CLASS", warpDropActive ? esc(activeWarpArtifact?.classification || activeWarpArtifact?.taxonomy?.category || activeWarpArtifact?.class || "SURFACED") : "???")}${kv("RARITY", warpDropActive ? `<span class="rarity ${esc(activeWarpArtifact?.rarity || "UNKNOWN")}">${esc(activeWarpArtifact?.rarity || "UNKNOWN")}</span>` : "???")}${kv("WEIRDNESS", warpDropActive ? `<span class="detail-weirdness weirdness-val ${weirdnessTier(activeWarpArtifact?.weirdness)}">${esc(activeWarpArtifact?.weirdness ?? "?")}</span>` : "???")}${kv("IDLE", warpDropActive ? esc(warpFunctions.idle) : "???")}${kv("INTENDED", warpDropActive ? esc(warpFunctions.intended) : "???")}${kv("ACCIDENTAL", warpDropActive ? esc(warpFunctions.accidental) : "???")}</div><p class=condition>${warpDropActive ? "✦ Drop is active! Click LOOT within the 1-minute window." : (warpStateClosed ? "Warp closed.<br>Vault queue is empty." : "Closing condition unknown.<br>Stay awhile and do whatever...")}</p><div class="participation-actions lot-action-height">${actionButtonHtml}</div>`;
          ...
          if (warpDropActive) {
            ...
            if (!warpStateClosed) {
              if (joinedWarpGimmick) {
                joinAttempts++;
                if (joinAttempts >= 3) showToast("WHATEVER...");
                else if (joinAttempts >= 2) showToast("THE VOID DOESN'T CARE FOR YOUR PERSISTENCE.");
                else showToast("YOUR PRESENCE IS ALREADY RECORDED.");
                return;
              }
              joinedWarpGimmick = true;
              warpJoinedAt = Date.now();
    ...
    function drawRecent(a) {
      ...
      for (let x of a) {
        ...
        c.innerHTML = `<div class="card-face card-front"><div class=cardart></div><div class=cardbody><b class="card-name rarity-name ${esc(x.rarity)}">${esc(x.name)}</b></div></div><div class="card-face card-back"><span class="void-orb-mark">◉</span><small class="recent-back-id">#${esc(x.artifact_id)}</small></div>`;
        c.querySelector(".cardart").append(cv(x));
        c.onclick = () => c.classList.toggle("is-flipped");
        let art = c.querySelector(".cardart");
        if (art) art.onclick = e => { e.stopPropagation(); collectionDetail(x, { hideProperties: true }); };
        let nb = c.querySelector(".card-name");
        if (nb) nb.onclick = e => { e.stopPropagation(); collectionDetail(x, { hideProperties: true }); };
        g.append(c);
      }
...
function drawArchive(a) {
  ...
  for (let x of a) {
    ...
    if (nb) nb.onclick = e => {
      ...
      collectionDetail(x, { hideProperties: true });
    };
    g.append(c);
  }
}
...
async function detail(id) {
      ...
      await archive();
      collectionDetail(a, { hideProperties: true });
    }
        ...
        <section class="panel wide fusion-panel">
          <div class="archive-top">
            <div>
              ...
              <div class="eyebrow">SEMANTIC SYNTHESIS &amp; BRIDGES</div>
          ...
          <div class="fusion-stage-grid">
            <!-- SLOT A (TRADE SBS COL STYLE) -->
            <div class="trade-sbs-col ${fusionSlotA ? 'active-choice' : ''}" id="slot-a-box"></div>

            <!-- FUSION CORE / BRIDGES (CENTER SECTION WITH INITIATE FUSION) -->
            ...
            <!-- SLOT B (TRADE SBS COL STYLE) -->
            <div class="trade-sbs-col ${fusionSlotB ? 'active-choice' : ''}" id="slot-b-box"></div>
          </div>
    ...
    function renderFusionSlots() {
      ...
      if (boxA) {
        if (fusionSlotA) {
          boxA.className = "trade-sbs-col active-choice";
          boxA.innerHTML = `
            <div class="trade-col-header">
              <small>ARTIFACT A (SOURCE CONSTRUCT)</small>
              <button class="understood-button" id="btn-choose-a" type="button" style="padding:2px 8px;font-size:9px">CHANGE</button>
            </div>
            <div class="card-title-group" style="margin-bottom:6px">
              <h3 class="rarity-name ${esc(fusionSlotA.rarity)}" style="margin:2px 0 0;font-size:16px">${esc(fusionSlotA.name)}</h3>
            </div>
            <div class="card-art-container" style="margin-bottom:8px">
              <div class="detailart trade-side-art" id="art-slot-a">
                <span class="card-face card-front"></span>
                <small class="trade-art-id">#${esc(fusionSlotA.artifact_id)}</small>
              </div>
            </div>
            <div class="collection-stats">
              ${rr("CLASSIFICATION", fusionSlotA.classification)}
              ${rr("ANOMALY", String(fusionSlotA.anomaly || "UNKNOWN").replaceAll("_", " "))}
              ${weirdnessRow(fusionSlotA.weirdness)}
            </div>
          `;
          boxA.querySelector("#art-slot-a .card-front").append(cv(fusionSlotA));
        } else {
          boxA.className = "trade-sbs-col";
          boxA.innerHTML = `
            <div class="trade-col-header">
              <small>ARTIFACT A (SOURCE CONSTRUCT)</small>
            </div>
            <div class="trade-empty-picker">
              <h3>CHOOSE AN ARTIFACT</h3>
              <p>Select an artifact from your vault to initiate fusion.</p>
        ...
        let bA = $("#btn-choose-a");
        if (bA) bA.onclick = () => openFusionPicker("A", candidates);
      }

      if (boxB) {
        if (fusionSlotB) {
          boxB.className = "trade-sbs-col active-choice";
          boxB.innerHTML = `
            <div class="trade-col-header">
              <small>ARTIFACT B (TARGET GESTALT)</small>
              <button class="understood-button" id="btn-choose-b" type="button" style="padding:2px 8px;font-size:9px">CHANGE</button>
            </div>
            <div class="card-title-group" style="margin-bottom:6px">
              <h3 class="rarity-name ${esc(fusionSlotB.rarity)}" style="margin:2px 0 0;font-size:16px">${esc(fusionSlotB.name)}</h3>
            </div>
            <div class="card-art-container" style="margin-bottom:8px">
              <div class="detailart trade-side-art" id="art-slot-b">
                <span class="card-face card-front"></span>
                <small class="trade-art-id">#${esc(fusionSlotB.artifact_id)}</small>
              </div>
            </div>
            <div class="collection-stats">
              ${rr("CLASSIFICATION", fusionSlotB.classification)}
              ${rr("ANOMALY", String(fusionSlotB.anomaly || "UNKNOWN").replaceAll("_", " "))}
              ${weirdnessRow(fusionSlotB.weirdness)}
            </div>
          `;
          boxB.querySelector("#art-slot-b .card-front").append(cv(fusionSlotB));
        } else {
          boxB.className = "trade-sbs-col";
          boxB.innerHTML = `
            <div class="trade-col-header">
              <small>ARTIFACT B (TARGET GESTALT)</small>
            </div>
            <div class="trade-empty-picker">
              <h3>CHOOSE AN ARTIFACT</h3>
              <p>Select a second artifact to synthesize in the fusion chamber.</p>
              <div class="trade-offer-actions">
        ...
        let bB = $("#btn-choose-b");
        if (bB) bB.onclick = () => openFusionPicker("B", candidates);
      }
    }
      ...
      app.innerHTML = `
        <section class="panel wide">
          <div class="archive-top">
            ...
            <div class="filters">
              <button class="understood-button" id="gen-filter-3d" style="padding:8px 14px;cursor:pointer;font-size:11px;color:#f0c354;border-color:#a87d2a;">✨ 3D MODELS</button>
      ...
      async function loadBatch(count = 100) {
        ...
        function filter() {
          ...
          for (let x of items) {
            ...
            c.innerHTML = `
              ...
              <div class="card-face card-back"><span class="void-orb-mark">◉</span><small class="archive-back-id">#${esc(x.artifactId)}</small></div>
            `;
            c.querySelector(".cardart").append(cv(x));
            c.onclick = () => c.classList.toggle("is-flipped");
            let nb = c.querySelector(".card-name");
        ...
        if (filter3dBtn) {
          filter3dBtn.onclick = () => {
            ...
            filter3dBtn.textContent = only3D ? "✓ SHOWING 3D" : "✨ 3D MODELS";
            filter();
          };
        }
        filter();
...
const ANOMALY_STATS_CATALOG = {
  ONLY_UNOBSERVED: (w) => [
    { name: "WITNESS TOLERANCE", value: Math.max(1, 101 - w), unit: "%" },
    { name: "UNSEEN ACTIVITY", value: w, unit: "%" },
    { name: "OBSERVED MOTION", value: 0, unit: "mm" }
  ],
  EMPTY_SIGNAL: (w) => [
    { name: "SOURCE DISTANCE", value: "UNKNOWN", unit: "" },
    { name: "RECEPTION", value: w, unit: "%" },
    { name: "TRANSMITTERS FOUND", value: 0, unit: "" }
  ],
  RETURNS_YESTERDAY: (w) => [
    { name: "TEMPORAL OFFSET", value: -1, unit: "day" },
    { name: "CORRECTION ATTEMPTS", value: 12 + Math.floor(w / 4), unit: "" },
    { name: "DRIFT", value: 0, unit: "sec/day" }
  ],
  WRONG_ROOM: (w) => [
    { name: "MEASURED DISPLACEMENT", value: (w / 17).toFixed(1), unit: "m" },
    { name: "VISIBLE DISPLACEMENT", value: 0, unit: "m" },
    { name: "AGREEING SENSORS", value: 3 + Math.floor(w / 25), unit: "" }
  ],
  REFUSES_PURPOSE: (w) => [
    { name: "IDLE FUNCTION", value: 100, unit: "%" },
    { name: "INTENDED FUNCTION", value: Math.max(0, 100 - w), unit: "%" },
    { name: "ACCIDENTAL FUNCTION", value: w, unit: "%" }
  ]
};

function getAnomalyStatsFallback(anomaly, weirdness) {
  const w = Number(weirdness) || 80;
  const raw = String(anomaly || "").toUpperCase().replace(/\s+/g, "_");
  if (ANOMALY_STATS_CATALOG[raw]) {
    return ANOMALY_STATS_CATALOG[raw](w);
  }
  const keys = Object.keys(ANOMALY_STATS_CATALOG);
  let hash = 0;
  for (let i = 0; i < raw.length; i++) hash = (hash * 31 + raw.charCodeAt(i)) >>> 0;
  const picked = keys[hash % keys.length];
  return ANOMALY_STATS_CATALOG[picked](w);
}
...
function collectionDetail(a) {
      ...
      let fromRecentOrArchive = Boolean(opts.hideProperties || opts.fromRecentOrArchive);
      let hideProperties = opts.hideProperties ?? (!owned);
      
      let actions = "";
      if (!fromRecentOrArchive) {
        if (a.claimable) {
          actions = `<a class="join archive-claim card-modal-claim" id=collection-dl href="/api/artifact/${encodeURIComponent(a.artifact_id || a.artifactId || "")}/download">CLAIM</a>`;
        } else if (owned) {
          actions = `<button class="understood-button" id="btn-quick-fusion" type="button">✦ FUSION</button><button class="understood-button" id="btn-quick-duel" type="button">⚔️ DUEL</button><button class="understood-button collection-remove" id="btn-remove-record" type="button">REMOVE RECORD</button>`;
        } else {
          actions = `<span class="collection-record-status">ARCHIVED RECORD</span>`;
        }
      } else if (a.claimable) {
        actions = `<a class="join archive-claim card-modal-claim" id=collection-dl href="/api/artifact/${encodeURIComponent(a.artifact_id || a.artifactId || "")}/download">CLAIM</a>`;
      }
      ...
      // Anomaly stats/values (oldVoid formulas)
      const statsList = (Array.isArray(a.stats) && a.stats.length) ? a.stats : getAnomalyStatsFallback(a.anomaly, a.weirdness);
      let anomalyStatsHtml = "";
      if (Array.isArray(statsList)) {
        const cardStatLabels = {
          "IDLE FUNCTION": "IDLE",
          "INTENDED FUNCTION": "INTENDED",
          "ACCIDENTAL FUNCTION": "ACCIDENTAL"
        };
        anomalyStatsHtml = statsList.map(s => rr(cardStatLabels[s.name] || s.name, `${s.value}${s.unit ? " " + s.unit : ""}`)).join("");
      }
      if (a.anomaly_domain) {
        anomalyStatsHtml += rr("ANOMALY DOMAIN", a.anomaly_domain);
      }
      ...
      let anomalyDescription = a.anomaly || "";
      let classification = a.classification || a.taxonomy?.category || a.taxonomy?.classType || "UNKNOWN";

      app.insertAdjacentHTML("beforeend", `
        <div class="modal" id="collection-modal">
          <div class="collection-detail-modal">
            <div class="collection-detail-flip-card flip-card" id="collection-detail-card">
              <div class="card-face card-front collection-detail-front">
              ...
              <div class="card-info">
                <div class="card-art-container collection-detail-art"><div class="detailart square-detail-art" id=collection-detail-art></div></div>
                ${anomalyDescription ? `<p class="lore card-description-text">${esc(anomalyDescription)}</p>` : ""}
                ...
                ${extraDetailsHtml}
                <div class="collection-stats">
                  ${rr("CLASS", classification)}
                  ${weirdnessRow(a.weirdness)}
                  ${a.hostChassis ? rr("HOST CHASSIS", a.hostChassis) : ""}
                  ${a.organicTaxon ? rr("ORGANIC TAXON", a.organicTaxon) : ""}
                  ${anomalyStatsHtml}
                  ${!fromRecentOrArchive && !hideProperties ? rr("INSTANCE REVISION", `REV ${inst.revision} (DUELS: ${inst.duels}, FUSIONS: ${inst.fusions})`) : ""}
                </div>

                ${!fromRecentOrArchive && !hideProperties ? `
                ...
                ${actions ? `<div class="collection-detail-actions">${actions}</div>` : ""}
              </div>
              </div>
              <div class="card-face card-back collection-detail-back">
                <small class="recent-back-id">#${esc(a.artifact_id || a.artifactId || "")}</small>
                <span class="void-orb collection-detail-back-orb">◉</span>
              </div>
            </div>
          </div>
        </div>
      ...
      if (detailArt) {
        detailArt.append(cv(a));
      }

      let detailCard = $("#collection-detail-card");
      if (detailCard) detailCard.onclick = event => {
...
function contribute() { let lotId = current?.id; sys("❓ ONE CONTRIBUTION"); app.innerHTML = `<section class="panel wide contribution"><div class=eyebrow>THE VOID WILL ACCEPT SOMETHING IN RETURN</div><div class=voidq>❓</div><h1>LEAVE SOMETHING IN THE VOID</h1><p>Leave an idea behind. You will not know when — or whether — it returns.</p><textarea id=idea maxlength=280 placeholder="e.g. a suspicious toaster that predicts rain"></textarea><div class=formrow><span><b id=chars>0</b>/280</span><button id=release>CAST INTO THE VOID</button></div></section>`; $("#idea").oninput = e => $("#chars").textContent = e.target.value.length; $("#release").onclick = async () => { await api("/api/contribute", { method: "POST", body: JSON.stringify({ text: $("#idea").value }) }); if (lotId) sessionStorage.setItem(`void_contributed_${lotId}`, "1"); app.innerHTML = `<section class="panel wide contribution"><div class=eyebrow>✦ CONTRIBUTION ACCEPTED</div><h1>IT IS NO LONGER YOURS.</h1><p>Destination unknown. Surface date unknown. Attribution none.</p><button id=back>RETURN TO THE VOID</button></section>`; $("#back").onclick = () => go("/void") } }
    ...
    async function vaultView() {
      ...
      const vaultUserEmail = sessionStorage.getItem("void_vault_user_email") || savedEmail || "byalov.v.martin@gmail.com";
      const vaultUserName = sessionStorage.getItem("void_vault_user_name") || "";

      if (!isVaultAuth) {
        app.innerHTML = `
          <section class="panel wide vault-studio" style="max-width:480px; margin:48px auto; text-align:center; padding:36px 28px;">
            <div class="eyebrow">RESTRICTED ACCESS // VAULT CONTROLLER</div>
            <h1 style="margin:12px 0 10px;">✦ SECURE VAULT ACCESS</h1>
            <p style="margin:0 auto 28px; color:#8ba2bf; max-width:400px; font-size:13px; line-height:1.6;">
              Вход с вашия Google профил за пълен контрол върху The Void Vault, инвентара и Google Drive.
            </p>

            <div style="margin-top:20px;">
              <button id="btn-vault-google-login" type="button" class="btn-primary-glow" style="display:flex; align-items:center; justify-content:center; gap:12px; width:100%; padding:14px 20px; font-size:14px; font-weight:800; cursor:pointer;">
        ...
        const loginWithAccount = async (email, name) => {
          const cleanEmail = (email || "byalov.v.martin@gmail.com").trim();
          ...
          sessionStorage.setItem("void_vault_user_email", cleanEmail);
          sessionStorage.setItem("void_vault_user_name", displayName);
          localStorage.setItem("void_vault_saved_email", cleanEmail);

          showToast(`✓ Успешен вход като ${cleanEmail}`);
          await vaultView();
        };

        // Single Google Login button: authenticates directly
        if ($("#btn-vault-google-login")) {
          $("#btn-vault-google-login").onclick = async () => {
            const btn = $("#btn-vault-google-login");
            btn.disabled = true;
            btn.textContent = "ВЛИЗАНЕ В СИСТЕМАТА...";
            await loginWithAccount(savedEmail || "byalov.v.martin@gmail.com", "Martin Byalov");
          };
        }
        return;
      }
      ...
      try {
        const [status, invRes, warpRes, registryRes, archiveRes, driveScanRes] = await Promise.all([
        ...
        if (driveScanRes?.images && Array.isArray(driveScanRes.images)) {
          storedGdriveImages = driveScanRes.images.map(x => x.name || `${x.artifactId}.png`);
          localStorage.setItem("void_gdrive_images", JSON.stringify(storedGdriveImages));
        } else if (registryArtifactCount > 0) {
          storedGdriveImages = registryRes.artifacts.map(a => a.name || `${a.artifact_id || a.id}.png`);
          localStorage.setItem("void_gdrive_images", JSON.stringify(storedGdriveImages));
        } else {
          try {
            storedGdriveImages = JSON.parse(localStorage.getItem("void_gdrive_images") || "[]");
          } catch {
            storedGdriveImages = [];
          }
        }
      ...
      const surfacedList = Array.isArray(archiveData?.entries) ? archiveData.entries : (Array.isArray(archiveData) ? archiveData : []);
      let lastSpawnedText = "✓ Stock is healthy";
      const lastSurfaced = warpData?.warp?.artifact || surfacedList[0] || null;
      if (lastSurfaced) {
        const lastId = lastSurfaced.artifact_id || lastSurfaced.id || lastSurfaced.artifactId;
        lastSpawnedText = `Last spawned: ${lastId}`;
      } else if (unspawnedCount < 10) {
        lastSpawnedText = "⚠️ LOW POOL — Time to add next batch";
      }
        ...
        <section class="panel wide vault-studio">
          ...
          <div class="panel-header-row">
            ...
            <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
              <div class="auth-status-badge connected" style="display:flex; align-items:center; gap:8px; background:#081728; border:1px solid #1b3756; padding:6px 12px; border-radius:6px;">
                <span class="dot" style="background:#4ade80; width:8px; height:8px; border-radius:50%; box-shadow:0 0 8px #4ade80;"></span>
                <span style="font-size:12px; font-weight:700; color:#e2ecf9;">${esc(vaultUserName ? `${vaultUserName} (${vaultUserEmail})` : vaultUserEmail)}</span>
              </div>
          ...
          <div class="vault-inventory-grid">
            <div class="inv-card">
              ...
              <span class="inv-sub" id="inv-unspawned-sub">${lastSpawnedText}</span>
            </div>
            <div class="inv-card">
              <span class="inv-label">PUBLIC ARCHIVE</span>
              <span class="inv-value" id="inv-archive">${archiveArtifactCount}</span>
            ...
            <div class="inv-card">
              ...
              <span class="inv-sub">Folder: 1kSLnRIJ-mmxhq2G1x2ikaUYo-sekvZxR<br><b style="color:#60a5fa">NEXT SERIAL: VA-${String(registryNextSerial).padStart(6, "0")}</b></span>
            </div>
            <div class="inv-card action-card">
              <span class="inv-label">WARP CONTROL</span>
              ${(warpData?.warp?.state === "WAITING" || warpData?.warp?.state === "ACTIVE")
                ? `<button id="btn-stop-warp" class="btn-danger-glow" type="button">⏹ STOP WARP</button>`
                : `<button id="btn-dispense-now" class="btn-primary-glow" type="button" ${unspawnedCount === 0 ? 'disabled' : ''}>▶ START WARP</button>`
              }
              <span class="inv-sub">${(warpData?.warp?.state === "WAITING" || warpData?.warp?.state === "ACTIVE") ? "WARP is active. Press to stop manually." : "Starts the global WARP cycle with the next unspawned artifact."}</span>
            </div>
            <div class="inv-card action-card">
              <span class="inv-label">TEST DROP</span>
              <button id="btn-force-drop" class="btn-primary-glow" type="button">
                ⚡ FORCED DROP
              </button>
              <span class="inv-sub">Immediately surfaces the next unspawned artifact</span>
            </div>
          </div>
          ...
          <div class="vault-tools-section" id="registry-maintenance">
            <div class="tool-header">
              ...
              <p>Destructive administrator controls for the canonical Vault Registry and public Archive.</p>
            </div>

            <div class="vault-inventory-grid" style="margin-top:14px; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));">
              <div class="inv-card action-card">
                <span class="inv-label">REMOVE LAST ARTIFACT</span>
                <button id="btn-remove-last-artifact" class="btn-primary-glow" type="button" ${registryArtifactCount === 0 ? "disabled" : ""}>
                  REMOVE LAST ARTIFACT
                </button>
                <span class="inv-sub">Removes the highest serial (VA-${String(Math.max(1, registryNextSerial - 1)).padStart(6, "0")}) from Google Drive and Vault Registry if not yet surfaced.</span>
              </div>
              <div class="inv-card action-card">
                <span class="inv-label">CLEAR VAULT REGISTRY</span>
                <button id="btn-clear-vault-registry" class="btn-danger-glow" type="button">
              ...
              <div class="inv-card action-card">
                <span class="inv-label">CLEAR ARCHIVE</span>
                <button id="btn-clear-archive" class="btn-danger-glow" type="button" ${archiveArtifactCount === 0 ? "disabled" : ""}>
                  CLEAR ARCHIVE
                </button>
          ...
          <div class="vault-upload-box">
            ...
            <p>Canonical artifact images and registry are stored through the protected Google Apps Script bridge.</p>
      ...
      if ($("#btn-vault-logout")) {
        $("#btn-vault-logout").onclick = async () => {
          ...
          sessionStorage.removeItem("void_vault_user_email");
          sessionStorage.removeItem("void_vault_user_name");
      ...
      // Stop WARP manually
      if ($("#btn-stop-warp")) {
        const stopBtn = $("#btn-stop-warp");
        stopBtn.onclick = async () => {
          stopBtn.disabled = true;
          stopBtn.textContent = "⏹ STOPPING...";
          try {
            await VoidAPI.stopWarp();
            warpDropActive = false;
            warpStateClosed = true;
            activeWarpArtifact = null;
            realExpiresAt = null;
            ...
            joinedWarpGimmick = false;
            warpJoinedAt = null;
            showToast("✓ WARP CYCLE STOPPED. THE VOID IS IDLE (WARP CLOSED).");
            await vaultView();
          } catch (err) {
            showToast("Stop Warp error: " + err.message);
            stopBtn.disabled = false;
            stopBtn.textContent = "⏹ STOP WARP";
          }
        };
      }

      // Start WARP with the next unspawned artifact
      if ($("#btn-dispense-now")) {
        ...
        forceBtn.onclick = async () => {
          ...
          try {
            ...
            showToast("Warp start error: " + err.message);
      ...
      // Forced Drop: surfaces next sequential unspawned artifact immediately
      if ($("#btn-force-drop")) {
        ...
        forceDropBtn.onclick = async () => {
          ...
          try {
            const surfacedSet = new Set(surfacedList.map(x => x.artifact_id || x.id || x.artifactId));
            const unspawnedArtifacts = (vaultRegistryData?.artifacts || []).filter(a => !surfacedSet.has(a.artifact_id || a.id || a.artifactId));

            let chosenId = null;
            if (unspawnedArtifacts.length > 0) {
              chosenId = unspawnedArtifacts[0].artifact_id || unspawnedArtifacts[0].id;
            }

            const result = await VoidAPI.forceDrop(chosenId ? { artifactId: chosenId } : {});
            const artName = result?.artifact?.name || "";
            showToast(`✦ SOMETHING SURFACED${artName ? ` — ${artName}` : ""}. 60 SECONDS TO LOOT.`);
            await checkPublicWarpOnce();
            go("/void");
          } catch (err) {
            showToast("Forced Drop error: " + err.message);
      ...
      if ($("#btn-remove-last-artifact")) {
        $("#btn-remove-last-artifact").onclick = async () => {
          ...
          try {
            try { await VoidAPI.stopWarp(); } catch {}
      ...
      if ($("#btn-clear-archive")) {
        $("#btn-clear-archive").onclick = async () => {
          ...
          try {
            ...
            homeArchive = [];
            showToast("✓ PUBLIC ARCHIVE CLEARED.");
            await vaultView();
          } catch (err) {
